...
in
{
  ...
  idx = {
    ...
    workspace = {
      ...
      onCreate = {
        # Install wait-on via npm globally
        install-wait-on = "npm install -g wait-on";
      ...
      onStart = {
        start-services = ''
          ...
          # Call 'wait-on' directly since it's now in the shell path via 'npm install -g'
          wait-on -t 30000 http-get://localhost:8080/ http-get://localhost:3001/api/auth/session || { echo "Backend services failed to start within 30 seconds." >&2; exit 1; }
          
          echo "All backend services are UP. Starting UI preview."
        '';
      };
    };
  };
